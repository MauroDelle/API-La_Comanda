...
class Mesa implements Ipersistencia
{
    ...
    public static function obtenerMesaMasFacturo()
    {
        $objDataAccess = DataAccess::getInstance();
        $consulta = $objDataAccess->prepareQuery("
            SELECT codigo_mesa, SUM(total) as totalFacturado
            FROM facturas
    ...
    public static function obtenerMesaMenosFacturo()
    {
        $objDataAccess = DataAccess::getInstance();
        $consulta = $objDataAccess->prepareQuery("
        SELECT codigo_mesa, SUM(total) as totalFacturado
    ...
    public static function obtenerMesaMayorImporteFacturado()
    {
        $objDataAccess = DataAccess::getInstance();
        $consulta = $objDataAccess->prepareQuery("
        SELECT codigo_mesa, MAX(total) as mayorImporte
        FROM facturas
        GROUP BY codigo_mesa
        ORDER BY mayorImporte DESC
        LIMIT 1
    ");
        $consulta->execute();
    ...
    public static function obtenerMesaMenorImporteFacturado()
    {
        $objDataAccess = DataAccess::getInstance();
        $consulta = $objDataAccess->prepareQuery("
        SELECT codigo_mesa, MIN(total) as menorImporte
        FROM facturas
        GROUP BY codigo_mesa
        ORDER BY menorImporte ASC
        LIMIT 1
    ");
        $consulta->execute();
